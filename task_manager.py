...
def display_task(task):
    ...
    print(f"{'Task:':<20}{task[1]}\n"
          f"{'Assigned to:':<20}{task[0]}\n"
          f"{'Date assigned:':<20}{task[3]}\n"
          f"{'Due date:':<20}{task[4]}\n"
          f"{'Task complete?':<20}{task[5]}\n"
          f"{'Task description:'}\n{task[2]}\n")
    print('\u2500'*100)
...
while not validate_user(username, password):
    print("\n\033[31mIncorrect username or password. Please try again\n\033[0m")
    username = input("Please enter your username:\n")
    password = input("Please enter your password:\n")
...
while True:
        ...
        if username != "admin":
            ...
            while True:
                ...
                if len(new_password) < 5:
                    print("\033[31m\nThe password is too short. It must be at least 5 characters long.\n\033[0m")
                    continue
            ...
            print(f"\033[32m\n{new_user} has been registered successfully\033[0m\n")
    ...
    # Add task section
        ...
        while task_user not in get_user_data():
            print("\033[31m\nInvalid username. Please provide an existing username.\n\033[0m")
            task_user = input("Please input the assigned username for the task:\n")
        task_title = input("Please add the task title:\n").strip()
        task_desc = input("Please enter the task description:\n").strip()
        task_due = input("Please enter the task due date \033[33m(DD MMM YYYY)\033[0m:\n").strip()
    
        with open("tasks.txt", "a+", encoding="utf-8") as tasks_file:
            tasks_file.write(f"\n{task_user}, {task_title}, {task_desc}, {format_today}, {task_due}, No")
    ...
    # Invalid section
        ...
        print("\033[31m\nYou have entered an invalid input. Please try again\033[0m\n")
